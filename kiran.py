...
async def cowsay(ctx, *args):
    await send_block(ctx, subprocess.run(('cowsay',) + args, stdout=subprocess.PIPE, universal_newlines=True, check=True).stdout)
...
async def on_message(message):
    try:
        ...
        if not isinstance(message.channel, discord.DMChannel) and 'muted' in message.channel.name.lower() and message.author.voice and not message.content.startswith('!'):
